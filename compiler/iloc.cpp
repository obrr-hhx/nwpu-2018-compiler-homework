...
Arm::Arm(string op, string rs, string arg1, string arg2, string add)
    :opcode(op),result(rs),arg1(arg1),arg2(arg2),addition(add),dead(false)
{}
...
void ILoc::init_var(Var* var, string initReg, string tmpReg){
    if(!var->unInit()){ // 变量初始化了
        if(var->isBase()){ // 基本类型初始化 int：100 char：‘a‘
...
void ILoc::ldr_var(string rsReg, Var*var){
    ...
    if(var->notConst()){// 非常量
        ...
        if(id!=-1){// 寄存器变量
            emit("mov", rsReg, Plat::regName[id]); // mov r8,r2
        }
        else{ // 所有定义的变量和数组
            int off=var->getOffset(); // 栈帧偏移
            bool isGlb = !off; // 变量偏移，0表示全局，其它表示局部
...
void ILoc::str_var(string srcReg, Var*var, string tmpReg){
    ...
    if(id!=-1){
        emit("mov",Plat::regName[id],srcReg);// mov r2,r8
    }
    else{
        int off=var->getOffset();
        bool isGlb=!off;
        if(isGlb){
            ldr_lb(tmpReg,var->getName()); // ldr r9,=glb
...
void ILoc::cmp(string cmp, string cmpnot, string rsReg, string reg1, string reg2){
    ...
    emit("mov"+cmp, rsReg,"#1"); // r8=1
    emit("mov"+cmpnot,rsReg,"#0"); // r8=0
    /*
        example:
            when the parameter cmp = gt, cmpnot = le
            the assembly code:
                cmp r8, r9
                movgt r8, #1
                movle r8, #0
            the mean is that r8 is 1 if r8 great than r9, otherwise is 0 
    */
}
